...
class SongUI(QWidget):
    ...
    def init_ui(self):
        ...
        # Create song list
        self.songs_table = SongArtistTableWidget(self)

        # Put it all together
        top_row = QHBoxLayout()
        top_row.addWidget(self.add_songs)
...
class FilterUI(QWidget):
        ...
        for ind in range(layout.count()):
            ...
            if isinstance(item, QWidgetItem):
                ...
                if isinstance(item.widget(), QComboBox):
                    ...
                    if depth == 1:
                        item_key = item.widget().currentText() + \
                                   '&&' + str(d_ind)
                        d_ind += 1
                        filt_dict[item_key] = {}
                        latest_pl = item_key
    ...
    def add_filter(self, logic_layout):
        ...
        # logic_layout.addWidget(new_filter, logic_layout.count(), 0)
        # logic_layout.addWidget(new_filter_remove, logic_layout.count() - 1, 1)
        # logic_layout.addWidget(new_filter_input, logic_layout.count() - 1, 0)
